...
import { CabinSeq, Dot, StatusBadge } from '../ui'
...
export function Workbench({ view, onEdit }: { view: ResultsView; onEdit: () => void }) {
  // Which rows are expanded. Start with the recommended one open; reset when a
  // new search comes in (a fresh `view` reference).
  const recRank = view.recIndex >= 0 ? view.rows[view.recIndex]?.rank : undefined
  const [expanded, setExpanded] = useState<Set<number>>(() =>
    new Set(recRank ? [recRank] : []),
  )
  useEffect(() => {
    setExpanded(new Set(recRank ? [recRank] : []))
      ...
      <div className="split-body">
        ...
        <div className="res-panel">
          ...
          <div className="stack" style={{ gap: 8 }}>
            {view.rows.map((row) => {
              const expandable = row.segs.length > 0
              const open = expanded.has(row.rank)
              return (
                <div className="rank-row" key={row.rank} style={{ border: `1px solid ${row.recBorder}` }}>
                  <div
                    className="rank-row-main"
                    onClick={() => expandable && toggle(row.rank)}
                    style={{ cursor: expandable ? 'pointer' : 'default' }}
                    role={expandable ? 'button' : undefined}
                    aria-expanded={expandable ? open : undefined}
                  >
                    <span className="rk-chevron">{expandable ? (open ? '▾' : '▸') : ''}</span>
                    <span className="rk">{row.rank}</span>
                    <span className="strat" style={{ color: row.stratColor }}>
                      {row.strat}
                    </span>
                    <CabinSeq flow={row.flow} />
                  ...
                  {open && expandable && (
                    <div
                      ...
                      style={
                        row.recFlag
                          ? undefined
                          : { background: 'var(--tint)', borderTopColor: 'var(--line)' }
                      }
                    >
                      {row.recFlag ? (
                        ...
                        <div style={{ fontSize: 12, color: 'var(--muted)' }}>
                          {row.priceStr} price
                          {row.creditStr !== '$0' && <> − {row.creditStr} comfort</>} ={' '}
                          <span style={{ color: 'var(--ink)', fontWeight: 600 }}>{row.effStr} effective</span> ·{' '}
                          {row.flag}
                        </div>
                      )}
                      <div className="stack" style={{ gap: 6 }}>
